...
struct ImGui_ImplGles3_Data
{
    ...
    ImGui_ImplGles3_Data() { memset((void*)this, 0, sizeof(*this)); }
};

// Backend data stored in io.BackendRendererUserData to allow support for multiple Dear ImGui contexts
// It is STRONGLY preferred that you use docking branch with multi-viewports (== single Dear ImGui context + multiple windows) instead of multiple Dear ImGui contexts.
static ImGui_ImplGles3_Data* ImGui_ImplGles3_GetBackendData()
{
    return ImGui::GetCurrentContext() ? (ImGui_ImplGles3_Data*)ImGui::GetIO().BackendRendererUserData : nullptr;
}
...
bool    ImGui_ImplGles3_Init()
{
    ...
    ImGui_ImplGles3_Data* bd = IM_NEW(ImGui_ImplGles3_Data)();
    io.BackendRendererUserData = (void*)bd;
    io.BackendRendererName = "imgui_impl_opengl3";

    bd->GlVersion = 310;  // GLES 3.1
...
{
    ImGui_ImplGles3_Data* bd = ImGui_ImplGles3_GetBackendData();
...
{
    ImGui_ImplGles3_Data* bd = ImGui_ImplGles3_GetBackendData();
    IM_ASSERT(bd != nullptr && "Did you call ImGui_ImplGles3_Init()?");
...
static void ImGui_ImplGles3_SetupRenderState(ImDrawData* draw_data, int fb_width, int fb_height, GLuint vertex_array_object)
{
    ImGui_ImplGles3_Data* bd = ImGui_ImplGles3_GetBackendData();
...
#endif
    ImGui_ImplGles3_SetupRenderState(draw_data, fb_width, fb_height, vertex_array_object);
...
static bool CheckShader(GLuint handle, const char* desc)
{
    ...
    if ((GLboolean)status == GL_FALSE)
        fprintf(stderr, "ERROR: ImGui_ImplGles3_CreateDeviceObjects: failed to compile %s! With GLSL: %s\n", desc, bd->GlslVersionString);
    if (log_length > 1)
    {
        ImVector<char> buf;
...
static bool CheckProgram(GLuint handle, const char* desc)
{
    ...
    if ((GLboolean)status == GL_FALSE)
        fprintf(stderr, "ERROR: ImGui_ImplGles3_CreateDeviceObjects: failed to link %s! With GLSL %s\n", desc, bd->GlslVersionString);
    if (log_length > 1)
    {
        ImVector<char> buf;
        buf.resize((int)(log_length + 1));
...
bool    ImGui_ImplGles3_CreateDeviceObjects()
{
    ...
    ImGui_ImplGles3_CreateFontsTexture();
