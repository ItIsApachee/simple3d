...
void CuboidInstance::BindAttributes() {
  ...
  for (int i = 0; i < model_rows; i++) {
    glVertexAttribPointer(
        model_layout_index + i, 4, GL_FLOAT, GL_FALSE, sizeof(CuboidInstance),
  ...
  glVertexAttribPointer(
      diffuse_color_layout_index, 3, GL_FLOAT, GL_FALSE, sizeof(CuboidInstance),
  ...
  if (reinitialize_instances) {
    ...
    for (auto& cuboid_ptr : updated_cuboids) {
      ...
      if (auto& index = cuboids_[cuboid_ptr].second; index == -1) {
        ...
        instances_[index] =
            CuboidInstance{model, cuboid.diffuse_color, cuboid.specular_color,
                           cuboid.shininess};
      }
    }
  }
